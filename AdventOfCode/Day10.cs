...
namespace AdventOfCode
{
    public class Day10
    {
        public void DoIt()
        {
            var botsFixed = new Dictionary<int, HashSet<int>>();
            var botsLowHigh = new Dictionary<int, Tuple<int, int>>();

            var fixedRegex = new Regex(@"value (\d+) goes to bot (\d+)");
            var lowHigh = new Regex(@"bot (\d+) gives low to (\w+) (\d+) and high to (\w+) (\d+)");

            foreach (var line in _input.SplitLines())
            {
                int botNum;

                var m = fixedRegex.Match(line);
                if (m.Success)
                {
                    var val = m.Groups[1].Value.ParseInt();
                    botNum = m.Groups[2].Value.ParseInt();
                    SetBotChip(botsFixed, botNum, val);
                    continue;
                }

                m = lowHigh.Match(line);
                if (m.Success)
                {
                    botNum = m.Groups[1].Value.ParseInt();
                    var low = m.Groups[3].Value.ParseInt();
                    var high = m.Groups[5].Value.ParseInt();

                    if (m.Groups[2].Value == "output")
                        low = -low - 1;

                    if (m.Groups[3].Value == "output")
                        high = -high - 1;

                    botsLowHigh[botNum] = Tuple.Create(low, high);
                }
                else
                    throw new Exception("Knas");
            }

            HashSet<int> botRet = new HashSet<int>();

            foreach (var bot in botsFixed)
            {
                if (bot.Value.Contains(61) && bot.Value.Contains(17))
                    botRet.Add(bot.Key);
            }
            ...
            {
                changed = false;
                foreach (var lowHighBot in botsLowHigh)
                {
                    if (botsFixed.ContainsKey(lowHighBot.Key) == false)
                        continue;
                    var bot = botsFixed[lowHighBot.Key];
                    if (bot.Count == 2)
                    {
                        changed |= SetBotChip(botsFixed, lowHighBot.Value.Item1, bot.Min());
                        changed |= SetBotChip(botsFixed, lowHighBot.Value.Item2, bot.Max());

                        if(botsFixed[lowHighBot.Value.Item1].Contains(61) && botsFixed[lowHighBot.Value.Item1].Contains(17))
                            botRet.Add(lowHighBot.Value.Item1);
                        if (botsFixed[lowHighBot.Value.Item2].Contains(61) && botsFixed[lowHighBot.Value.Item2].Contains(17))
                            botRet.Add(lowHighBot.Value.Item2);
            ...
            Tools.PostResult(botsFixed[-1].Single() * botsFixed[-2].Single()*  botsFixed[-3].Single());
            //PostResult(botRet.Count);
        }

        private static bool SetBotChip(Dictionary<int, HashSet<int>> botsFixed, int botNum, int val)
        {
            if (botsFixed.ContainsKey(botNum))
                return botsFixed[botNum].Add(val);

            botsFixed[botNum] = new HashSet<int>(new[] { val });
            return true;
        }

        private string _inputTest = @"value 5 goes to bot 2
