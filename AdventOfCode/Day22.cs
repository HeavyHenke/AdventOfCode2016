...
namespace AdventOfCode
{
    internal class Day22
    {
        public void DoItA()
        {
            var nodes = new Dictionary<(int x, int y), (int used, int avail)>();

            var regEx = new Regex(@"/dev/grid/node-x(?<x>\d+)-y(?<y>\d+)\s+(?<size>\d+)T\s+(?<used>\d+)T\s+(?<avail>\d+)");
            foreach(var line in _data.SplitLines().Skip(2))
            {
                var m = regEx.Match(line);
                var key = (int.Parse(m.Groups["x"].Value), int.Parse(m.Groups["y"].Value));
                var value = (int.Parse(m.Groups["used"].Value), int.Parse(m.Groups["avail"].Value));
                nodes.Add(key, value);
            }
            ...
            foreach (var a in nodes)
            {
                ...
                foreach (var b in nodes)
                {
                    if (a.Key == b.Key)
                        continue;
                    if (b.Value.avail >= a.Value.used)
                        numMatches++;
                }
            }

            Tools.PostResult(numMatches);
        }

        /*
        public void DoItB()
        {
            var startNode = new SearchNode(_data);

            for (int maxDepthThisIter = 0; maxDepthThisIter < 10000000; maxDepthThisIter++)
            {
                Console.WriteLine($"Testing @ depth {maxDepthThisIter}");
                
                var searchQueue = new Stack<SearchNode>();
                searchQueue.Push(startNode);

                while (searchQueue.Count > 0)
                {
                    var node = searchQueue.Pop();
                    ...
                    {
                        var available = node.Nodes[x, y].avail;
                        foreach (var neighbor in GetValidNeighbors((x, y))) // Flytta från neigbour till (x,y)
                        {
                            var (movedDataAmount, avail) = node.Nodes[neighbor.x, neighbor.y];
                            if (movedDataAmount <= available)
                            {
                                var child = new SearchNode(node);

                                child.Nodes[neighbor.x, neighbor.y] = (0, movedDataAmount + avail);

                                var dest = child.Nodes[x, y];
                                child.Nodes[x, y] = (dest.used + movedDataAmount, dest.avail - movedDataAmount);

                                if (neighbor == child.GoalPos)
                                    child.GoalPos = (x, y);
                                if (child.GoalPos == (0, 0))
                                {
                                    Console.WriteLine($"Found it after {child.Depth} moves");
                                    return;
                                }

                                if (child.Depth <= maxDepthThisIter)
                                    children.Add(child);
                            }
                        }
                    }
                    
                    foreach(var child in children.OrderByDescending(c => c.GoalPos.x + c.GoalPos.y))
...
*/
        ...
        public void DoItB2()
        {
            var startNode = new SearchNode(_test);
            
            var allWaysToGoal = Solve(startNode);
            
            Console.WriteLine($"Solution found in {allWaysToGoal.Depth} steps");
        }

        private SearchNode Solve(SearchNode node)
        {
            var searchList = new List<SearchNode> { node };
            var visited = new HashSet<(int x, int y)>();
            visited.Add(node.GoalPos);
                
            while (searchList.Count > 0)
            {
                var nodeTup = searchList.Index()
                    .OrderBy(c => c.Value.GoalPos.x + c.Value.GoalPos.y)
                    .ThenBy(d => d.Value.Depth)
                    .First();
                searchList.RemoveAt(nodeTup.Key);

                searchList.AddRange(MoveData(nodeTup.Value, nodeTup.Value.GoalPos, visited));
            }

            throw new Exception("Solution not found");
        }
        
        private IEnumerable<SearchNode> MoveData(SearchNode start, (int x, int y) posToMove, HashSet<(int x, int y)> dontMoveHere)
        {
            var (dataToMove, sourceAvailable) = start.Nodes[posToMove.x, posToMove.y];
            foreach (var neighbor in GetValidNeighbors(posToMove).Except(dontMoveHere))
            {
                if (start.Nodes[neighbor.x, neighbor.y].avail >= dataToMove)
                {
                    var child = new SearchNode(start, posToMove, neighbor);
                    yield return child;
                }
                else
                {
                    if(start.Nodes[neighbor.x, neighbor.y].avail + start.Nodes[neighbor.x, neighbor.y].used < dataToMove)
                        continue;
                    
                    var children = new List<SearchNode>();
                    // Move all data from neighbor to a neighbor of neighbor, neighbor-neighbor

                    var dontMoveHere2 = new HashSet<(int x, int y)>();
                    dontMoveHere2.Add(posToMove);
                    
                    foreach (var child in MoveData(start, neighbor, dontMoveHere2))
                    {
                        if (child.Nodes[neighbor.x, neighbor.y].avail >= dataToMove)
                            children.Add(new SearchNode(child, posToMove, neighbor));
                    }

                    var orderedChildren = children
                        .OrderByDescending(c => c.GoalPos.x + c.GoalPos.y)
                        .ThenBy(d => d.Depth);
        ...
        private static readonly (int, int)[] _delta = { (-1, 0), (1, 0), (0, -1), (0, 1) };

        private static int _maxX;
        private static int _maxY;

        private static IEnumerable<(int x, int y)> GetValidNeighbors((int x, int y) pos)
        {
            foreach (var d in _delta)
            {
                var p2 = (x: pos.x + d.Item1, y: pos.y + d.Item2);
                if (p2.x >= 0 && p2.x <= _maxX && p2.y >= 0 && p2.y <= _maxY)
                    yield return p2;
            }
        }
        private class SearchNode
        {
            //public readonly Dictionary<(int x, int y), (int used, int avail)> Nodes;
            public readonly (int used, int avail)[,] Nodes;
                //Dictionary<(int x, int y), (int used, int avail)> Nodes;

            public int Depth { get; }

            public (int x, int y) GoalPos { get; set; }

            public SearchNode(string input)
            {
                var nodesDict = new Dictionary<(int x, int y), (int used, int avail)>();
                var regEx = new Regex(@"/dev/grid/node-x(?<x>\d+)-y(?<y>\d+)\s+(?<size>\d+)T\s+(?<used>\d+)T\s+(?<avail>\d+)");
                foreach (var line in input.SplitLines().Skip(1))
                {
                    var m = regEx.Match(line);
                    var key = (x: int.Parse(m.Groups["x"].Value), y: int.Parse(m.Groups["y"].Value));
                    var value = (int.Parse(m.Groups["used"].Value), int.Parse(m.Groups["avail"].Value));
                    nodesDict.Add(key, value);
                ...
                Nodes = new (int used, int avail)[_maxX + 1, _maxY + 1];
                foreach (var kvp in nodesDict) 
                    Nodes[kvp.Key.x, kvp.Key.y] = kvp.Value;

                Depth = 0;
                GoalPos = (_maxX, 0);
            }

            public SearchNode(SearchNode other, (int x, int y) moveFrom, (int x, int y) moveTo)
            {
                Nodes = ((int used, int avail)[,])other.Nodes.Clone();
                Depth = other.Depth + 1;

                var moveFromData = Nodes[moveFrom.x, moveFrom.y];
                var moveToData = Nodes[moveTo.x, moveTo.y];

                Nodes[moveFrom.x, moveFrom.y] = (0, moveFromData.avail + moveFromData.used);
                Nodes[moveTo.x, moveTo.y] = (moveToData.used + moveFromData.used, moveToData.avail - moveFromData.used);
        ...
        private const string _test = @"Filesystem            Size  Used  Avail  Use%
...
/dev/grid/node-x2-y2    9T    6T     3T   66%";
        ...
        private const string _data = @"Filesystem              Size  Used  Avail  Use%
