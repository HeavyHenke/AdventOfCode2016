...
namespace AdventOfCode
{
    public class Day7
    {
        public void DoItA()
        {
            var outsideAbba = new Regex(@"(\w)(?!\1)(\w)\2\1(?![^\[]*\])");
            var insideAbba = new Regex(@"(\w)(?!\1)(\w)\2\1(?=[^\[]*\])");

            int numMatches = 0;
            foreach (var input in _input.Split(new[] { "\r\n" }, StringSplitOptions.None))
            {
                var insideMatch = insideAbba.IsMatch(input);
                var outsideMatch = outsideAbba.IsMatch(input);
                if(outsideMatch && !insideMatch)
                    numMatches++;
            }
        ...
        public void DoItB()
        {
            ...
            var abba = new Regex(@"(\w)(?!\1)(\w)\2\1");
            //var aba = new Regex(@"(\w)(?!\1)(\w)(?=\1)");

            int numMatches = 0;
            foreach (var input in _input.Split('\n'))
            {
                ...
                foreach (var m in match)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if(str[i] == str[i+2])
                                hypernet.Add(new string(new[] {str[i+1], str[i], str[i+1]}));
                        }
                    }
                }
        ...
        private string _inputTest = @"abba[mnop]qrst
abcd[bddb]xyyx
aaaa[qwer]tyui
ioxxoj[asdfgh]zxcvbn";

        private string _input = @"xdsqxnovprgovwzkus[fmadbfsbqwzzrzrgdg]aeqornszgvbizdm
