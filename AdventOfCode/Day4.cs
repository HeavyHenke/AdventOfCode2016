...
namespace AdventOfCode
{
    public class Day4
    {
        public void DoItA()
        {
            ...
            var chiper = new Dictionary<char, char>();
            var alphabet = "abcdefghijklmnopqrstuvwxyz";

            for (int i = 0; i < alphabet.Length - 1; i++)
            {
                chiper[alphabet[i]] = alphabet[i + 1];
            }
            chiper[alphabet[alphabet.Length - 1]] = alphabet[0];
            chiper['-'] = ' ';
            chiper[' '] = ' ';

            foreach (var test in input.Split('\n'))
            {
                var regex = new Regex(@"(?<text>.*)-(?<id>\d*)\[(?<checksum>\w{5})\]");

                var m = regex.Match(test);
                var q = from t in m.Groups["text"].Value
                    where char.IsLetter(t)
                    group t by t
                    into t2
                    select new { chr = t2.Key, cnt = t2.Count() };
                var q2 =
                    new string(q.OrderByDescending(a => a.cnt).ThenBy(b => b.chr).Take(5).Select(c => c.chr).ToArray());

                if (q2 == m.Groups["checksum"].Value)
                {
                    var sectorId = int.Parse(m.Groups["id"].Value);
                    totalId += sectorId;
                }
        ...
        public void DoItB()
        {
            ...
            foreach (var test in input.Split('\n'))
            {
                ...
                {
                    var sectorId = int.Parse(m.Groups["id"].Value);
                    string s2 = m.Groups["text"].Value;
                    for (int a = 0; a < sectorId; a++)
                    {
                        s2 = new string(s2.Select(c => chiper[c]).ToArray());
                    }

                    if (s2.Contains("pole"))
                    {
                        Console.WriteLine(s2 + " " + sectorId);
                        PostResult(sectorId);
                        return;
                    }
                }
        ...
        private static string input = @"aczupnetwp-dnlgpyrpc-sfye-dstaatyr-561[patyc]
